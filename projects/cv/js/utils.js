...
const Utils = {
  ...
  showToast(message, type = 'success') {
    ...
    // Escape message content to avoid injecting untrusted HTML
    const safeMessage = Utils && typeof Utils.escapeHtml === 'function' ? Utils.escapeHtml(message) : String(message || '');
    toast.innerHTML = `
      ${icon}
      <span class="text-sm font-medium text-gray-900">${safeMessage}</span>
    `;
  ...
  /**
   * Safely set innerHTML of an element by stripping <script> tags and unsafe attributes
   * This does a basic sanitization in-place to prevent XSS from template output.
   */
  sanitizeAndSetInnerHTML(container, html) {
    if (!container) return;

    // Use a template element to parse the HTML
    const tpl = document.createElement('template');
    tpl.innerHTML = html || '';

    // Walk the nodes and remove dangerous elements/attributes
    const walk = (node) => {
      ...
      // Remove attributes starting with 'on' (event handlers) and dangerous href/src values
    ...
    [...tpl.content.children].forEach(child => walk(child));
