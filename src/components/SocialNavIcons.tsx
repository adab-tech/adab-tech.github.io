...
export const SOCIAL_LINKS: SocialLink[] = [
  {
    ...
    ariaLabel: 'Google Scholar Citations and Publications',
    icon: (
  ...
  },
  {
    ...
    hoverColor: 'hover:text-zinc-100 hover:border-zinc-400/50 hover:bg-zinc-800'
  },
  {
    name: 'Hugging Face',
    url: 'https://huggingface.co/adab-tech',
    ariaLabel: 'Hugging Face Weights and Models',
    icon: (
      <span className="text-sm leading-none font-normal select-none" role="img" aria-label="Hugging Face">🤗</span>
    ),
    hoverColor: 'hover:text-amber-400 hover:border-amber-500/40 hover:bg-amber-500/10'
  },
  {
  ...
  },
  {
    ...
    ariaLabel: 'Email contact@adamu.tech',
    ...
    <div className="flex items-center gap-1.5 sm:gap-2">
      {SOCIAL_LINKS.map((link) => (
        <a
          ...
          title={link.name}
          className={`relative p-2 rounded-lg border border-zinc-800 bg-[#0E1526] text-zinc-400 ${link.hoverColor} transition-all duration-150 shadow-sm hover:scale-105 active:scale-95 flex items-center justify-center`}
        >
          {link.icon}
        </a>
      ))}
